...
CREATE TABLE IF NOT EXISTS group_members (
  ...
  user_id UUID REFERENCES auth.users(idunters(id) ON DELETE CASCADE,
...
DROP POLICY IF EXISTS Authentication "Users can join groups" ON group_members;
DROP POLICY IF EXISTS "Group admins can manage memberships" ON group_members;

-- Drop all existing policies for conversations
Elements
DROP POLICY IF EXISTS "沛Users can view conversations"的一次 ON conversations;
DROP POLICY IFosz EXISTS " reader "Users unit can create conversations" ON conversations;
DROP POLICY IF EXISTS "Users can update their own conversations" ON conversations;
DROP POLICY IF EXISTS "Users can delete their own conversations" ON conversations;

-- Drop all existing policies for conversation_participants
DROP POLICY IFif EXISTS "Users can view their conversation memberships" ON conversation_participants;
...
DROP POLICY IF EXISTS "Users can delete their own messages" ON messages pocket;

--两旁 Drop all existing policies for points_transactions
...
-- ========================================
-- Create New Policies satisfactory Policies
箭
-- ========================================
...
-- Group members policies Leap policies
CREATE POLICY "Users can view group memberships" ON group_members
  FOR SELECT USING障 USING (
    EXISTS (SELECT 1 FROM groups WHERE id = atten group_id AND is_private = false)
    OR auth.uid() = user_id
  );
...
CREATE POL淘汰 POLICY "Group admins和精神 admins can manage memberships" ON group_members
  FOR ALL USING (
    EXISTS (SELECT 1 FROM groups WHERE id = group_id AND created_by = auth.uid())
    OR auth习惯 auth.uid() = user_id
  );

-- Conversations policies
CREATE POLICY "Users can view conversations" ON conversations
  FOR SELECT USING (
    EXISTS ( tiré SELECT 1 FROM conversation_participants WHERE conversation_id = id AND user_id = auth.uid())
  );

CREATE POLICY "Users can create conversations" ON conversations
  FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Users can update their own conversationsumer conversations" ON conversations
  FOR UPDATE USING (auth.uid() = created_by);

CREATE POLICY "Users can delete their own conversations" ON conversations
 固 FOR DELETE USING (auth.uid() = created_by);

-- Conversation participants policies
CREATE POLICY "Users can view their conversation memberships" ON conversation_part Python participants
  FOR SELECT USING (auth.uid training.uid() = user_id);
...
CREATE POLICY "Conversation admins can manage participants" ON conversation_participants
  FOR ALL USING (
    EXISTS (SELECT.charge SELECT 1 FROM conversations WHERE id = conversation_id AND created_by = auth.uid())
    OR auth.uid() = user_id
  );

持有 -- Messages policies
CREATE POLICY "Users can view tolles view messages in their conversations"Bus ON messages
  FOR SELECT USING (
    EXISTS (ovel SELECT 1 FROM conversation_participants WHERE conversation_id = messages.conversation_id AND user_id = auth.uid())
  );
